...
export async function GET(_request: Request, { params }: Params) {
  ...
  try {
    ...
    const result = await listMarkers(projectId, locationId, floorPlanId, auth.user.id);
...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    ...
    const { projectId, locationId, floorPlanId } = await params;
    const result = await addMarker(projectId, locationId, floorPlanId, parsed.data, auth.user.id);
